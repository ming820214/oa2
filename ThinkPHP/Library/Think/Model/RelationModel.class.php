...
//                              if (!empty($val['relation_deep'])){ 此处关联层级没有起到应有的作用，真正的深层需要进行修改， edit by zhangxm at 2015-11-27 14:36,在后面递归后面还需要对相应的返回结果数据进行一些相应的判断与修改 在227行附近
								if (!empty($val['relation_deep']) && !empty($relationData)){ 
                        ...
                        if(!$return){
                            ...
                            }elseif(!empty($relationData)){
                            	
									$result[$mappingName] = $relationData;
									
							}
                            
                            /*此处没有对空结果集进行处理，我在此处添加对空结果集的判断处理 edit by zhangxm at 2015-11-27 14:41 在151行有对空数据集的判断
							 * else{
                                $result[$mappingName] = $relationData;
                            }*/
