...
class ApplyClassApproveController extends HomeController {
...
*/
	public function write(){
		...
		$config = array(
		  'maxSize'    =>    10485760,
		  'rootPath'   =>    './Uploads/',
		  'savePath'   =>    'ClassApprove/' . session('school_id') . '/',
		...
		if(I('post.id')){
		    ...
		    if($_FILES['references']['name'][0]){
		        ...
		        if(!$info) {// 上传错误提示错误信息
		            ...
		            foreach($info as $k=>$v){
		                if($v['key'] == 'certificate_pic'){
		                    $certificate_pic .= './Uploads/' . $v['savepath'].$v['savename'] . ';';
		                }else if($v['key'] == 'photos'){
		                    $photos .= './Uploads/' . $v['savepath'].$v['savename'] . ';';
		                }
		            }
		            
		            $mod->certificate_pic = $certificate_pic;
		            $mod->photos = $photos;
		...
		$mod->school_name=get_school_name();
		$mod->add_user=session('auth_id');
		$mod->add_user_name=session('user_name');
		
		if($_FILES['certificate_pic']['name'][0]){
		    // 上传文件
		    $info   =   $upload->upload();
		    if(!$info) {// 上传错误提示错误信息
		        $this->error('保存失败：' . $upload->getError());
...
*/
    public function ajax_list(){
    	...
    	if(IS_AJAX){
    		...
    		if($w['add_user_name']){
    		  $w['add_user_name'] = array('like','%'. $w['add_user_name'] . '%');
    		}
    		...
    		if(strpos(strstr($_SERVER['HTTP_REFERER'],'&a='),'manage') === FALSE){
         		 ...
         		 if(get_school_name()!='集团'){
         		     ...
         		     if(session('position_id') == '10'){
         		         //校长
         		         $w['state'] = 10;
         		     }elseif(session('position_id') == '1466'){
         		         //校长
         		         unset($w['school']);
         		         $w['state'] = 20;
         		     }else{
    		...
    		$data=M('applyClass')->where($map)->order('id desc')->field('id, state, school, apply_date, subject, student, stu_school, grade, wl, certificate_type, certificate_count, parent_type, tel, prev_score, prev_exact_rate, cur_score, cur_exact_rate, improve_num, improve_rate, exam_type, certificate_pic, photos, why, create_time, update_time, is_del, add_user, add_user_name, back, parent_type_other, school_name')->limit(I('get.offset'),I('get.count'))->select();
    		
    		  foreach ($data as &$vo){
    		    
    		    if($vo['certificate_pic']){
    		        $hrefs = explode(";",$vo['certificate_pic']);
    		        $ref = '';
    		        foreach($hrefs as $arr){
    		            $ref .= '<a href="' . $arr . '" target="_blank">' . $arr . '</a><br/>';
    		        }
    		        $vo['certificate_pic'] = $ref;
    		    }
    		    
    		    
    		    if($vo['photos']){
    		        $hrefs = explode(";",$vo['photos']);
    		        $ref = '';
    		        foreach($hrefs as $arr){
    		            $ref .= '<a href="' . $arr . '" target="_blank">' . $arr . '</a><br/>';
    		        }
    		...
    		$total=M('applyClass')->where($w)->count();
