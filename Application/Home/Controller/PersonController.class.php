...
class PersonController extends HomeController {
    ...
    //过滤查询字段
    function map_search() {
        I('param.name')?$map['name'] = array('like', "%" . I('param.name') . "%"):'';
        I('param.card')?$map['card'] = array('like', "%" . I('param.card') . "%"):'';
        I('param.part')?$map['part'] = I('param.part'):'';
        I('param.position')?$map['position'] = I('param.position'):'';
        return $map;
    }
...
*/
    ...
    public function _ask_import($class){
        ...
        $w['class']=$class;
        if(get_school_name()!='集团')$w['school']=get_school_name();
...
*/
    ...
    public function index(){
        ...
        $page= new \Home\Hongwen\Page($count,20);
        $show=$page->show();
        $list=$user->where($map)->order('id')->limit($page->firstRow.','.$page->listRows)->select();
        foreach ($list as $k => $v) {
            $listt[$k]=$v;
            $listt[$k]['info']=M('hw003.person_info',null)->find($v['id']);
        }
        $this->list=$listt;
        $this->page=$show;
        $this->display();
    ...
    public function lizhi(){
        if(IS_AJAX){
            if($_POST['b_c']){
                M('hw003.person_record',null)->where(array('id'=>I('post.id')))->save(array('b_c'=>I('post.b_c')));
            }elseif ($_POST['fz']) {
                M('hw003.person_all',null)->where(array('id'=>I('post.fz')))->setField('state',1);
                M('hw003.person_record',null)->where(array('pid'=>I('post.fz')))->setField('hide',1);
    ...
    public function record(){
        if($_POST['pid']){
            ...
            if($_POST['aa']=='离职'){
                ...
                $name=M('hw003.person_all',null)->where($w)->find();
                M('user')->where(array('name'=>$name['name']))->setField('is_del',1);//禁用oa账号
                M('hw001.user',null)->where(array('user'=>['like',$name['name']."%"]))->delete();//删除排课系统账号
                $wx= getWechatObj();
                $wx->deltUserInfo($name['userid']);
            }
    ...
    public function birthday(){
        $map=self::map_search();
        $map['state'] = 1;
        $cal=new \Home\Hongwen\Calendar();
        $m=M('hw003.person_all',null)->where($map)->getField('id,name,sex,position,part,school',true);
...
//==============仅供员工管理的相关部分调用
    function search($m){//获取到查询的结果，首次过滤
        foreach ($m as $v) {
            $k=$v;
            $m2=M('hw003.person_info',null)->find($v['pid']);
            $m3=M('hw003.person_all',null)->find($v['pid']);
            $k['card']=$m2['card'];
            $k['sex']=$m2['sex'];
            $k['part']=$m3['part'];
            $k['name']=$m3['name'];
...
*/
    ...
    public function kq_rule(){
        if(IS_AJAX){
            ...
            if($_GET['queren']){
                if(M("hw003.person_kq_rule",null)->where(array('id'=>$_GET['queren']))->setField('state',1))
                $this->ajaxReturn('ok');
            }
        }
        if($_GET['delt']){
            M("hw003.person_kq_rule",null)->where(array('id'=>$_GET['delt']))->setField('is_del',1);die;
        }
        if($_POST['ruleadd']){
        ...
        $w['school']=['in',[get_school_name(),'']];
        if($_GET['school'])$w['school']=$_GET['school'];
        $w['is_del']=0;
    ...
    public function kq_rules(){
        $school=$_GET['school']?$_GET['school']:get_school_name();
        //获取rules
        if(IS_AJAX){
            //修改时获取考勤方案
            if($_GET['rules']){
            ...
            //锁定规则组
            if($_GET['close']){
                if(M("hw003.person_kq_rules",null)->where(array('id'=>$_GET['close']))->setField('state',1))$this->ajaxReturn('ok');
            }
        }
        //删除方案组
        if($_GET['delt']){
        ...
        if($_GET['syid']){
            ...
            unset($d['id'],$d['timestamp'],$d['state']);
        ...
        if($_POST['bangding']){
            ...
            $m=M('hw003.person_all',null)->where($w)->getField('id',true);
            foreach ($m as $v) {
                $d['date']=session('date');
                $d['pid']=$v;
                $ruled=M('hw003.person_kq_ruled',null)->where(array('date'=>session('date'),'pid'=>$v))->find();
                if($ruled){
                    $dd['rules']=$_POST['rules'];
                    if($ruled['rules']==0)M('hw003.person_kq_ruled',null)->where($d)->save($dd);
                }else{
                    $d['rules']=$_POST['rules'];
    ...
    public function kq_ruled(){
        if(IS_AJAX){
            if(I('get.reset')){//解除规则绑定
                if(M('hw003.person_kq_ruled',null)->where(['date'=>session('date'),'pid'=>I('get.reset')])->setField('rules',0))$this->ajaxReturn('ok');
            }
        }
        $w=self::map_search();
        $w['school']=$w['school']?$w['school']:get_school_name();
        ...
        if($aa) {
            if($aa=='aa'||$aa=='bb'||$aa=='cc'||$aa=='dd'){
                ...
                foreach ($l as  $v) {
                    ...
                    $tr.='<tr><td>'.$v.'</td><td>'.date('H:i:s',$kq['t1']).'</td><td>'.(($kq['t11'])?date('H:i:s',$kq['t11']):'无').'</td><td>'.date('H:i:s',$kq['t2']).'</td><td>'.(($kq['t22'])?date('H:i:s',$kq['t22']):'无').'</td><td><a class=\'btn btn-xs btn-danger\' onclick='.'"kq_delt(this,\''.$aa.'\',\''.$v.'\','.$pid.')"'.'>不记入</a></td></tr>';
                ...
                if($ask){
                    foreach ($ask as $v) {
                        $tr.='<tr><td>'.$v['time1'].'</td><td>'.$v['time2'].'</td><td>'.$v['gong'].'</td><td>'.$v['info'].'</td><td width=\'70px;\'>'.$v['state'].'</td><td width=\'110px;\'>'.(($v['kq']==0)?'<a class=\'btn btn-xs btn-danger\' onclick=\'qingjia_jiaban(this,'.$pid.','.$v['id'].',1)\'>记入</a>-<a class=\'btn btn-xs btn-danger\' onclick=\'qingjia_jiaban(this,'.$pid.','.$v['id'].',2)\'>不记</a>':(($v['kq']==1)?'计入':'不计')).'</td></tr>';
                    }
                }
            }
        }
    ...
    public function kq_kq_import(){
        if(I('post.school')){
            ...
            $w['school']=I('post.school');
            $m=M('hw003.person_all',null)->where($w)->getField('id,name,school,position',true);
            foreach ($m as $k => $v) {
    ...
    public function ajax_kq_qingjia($id,$pid,$state){
        $m=M('hw003.person_ask',null)->find($id);
...
*/
    ...
    public function jl(){
        ...
        $mod=M('hw003.person_jl',null);
        $w['gs']=get_school_name();
        if(get_school_name()=='集团')unset($w['gs']);
        if($_POST['add']){
            $mod->create();
            if(I('post.id')){
                $mod->save();
        ...
        if(I('param.gs'))$w['gs']=I('param.gs');
        if(I('param.state'))$w['state']=I('param.state');
        if(I('param.name'))$w['name']=I('param.name');
        if(I('param.position'))$w['position']=I('param.position');
        if(I('param.city'))$w['city1|city2']=I('param.city');
        if(I('param.t1')&&I('param.t2'))$w['timestamp']=['BETWEEN',[I('param.t1').' 00:00:00',I('param.t2').' 00:00:00']];
        if($_GET['delt']){
            $mod->delete((int)$_GET['delt']);
        }
        ...
        $this->data=$mod->where($w)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->display();

    }

    public function ajax_jl($id){
        $this->ajaxReturn(M('hw003.person_jl',null)->find($id));
    }
