...
class XueguanController extends HomeController {
    ...
    public function student2(){
        ...
        // $m=M('hw001.class',null)->where($w)->getField('id,stuid',true);
        // if($m){
        //     $v=array_unique();
        ...
        // }
        // if($m&&$w2)$data=M('hw001.student',null)->where($w2)->getField('id,name,grade,type,xueguan,jiaoxue,state',true);
        // $this->list=$data;
        $this->display();
...
//==============共用box调用=========================
    ...
    public function fankui_record($stuid){
        ...
        foreach ($m as $v) {
            $data[]=array('上课日期：'.$v['date'].'...科目：'.$v['class'].'...讲师：'.$v['teacher'],'模块：'.$v['fk_a'].'；内容：'.get_config('TRACK_COURSE')[$v['fk_b']].'；状态：'.$v['fk_c'].'；亮点：'.$v['fk_d'].'；作业：'.$v['fk_e'].',作业'.$v['fk_f'],'沟通需要：'.$v['fk_g'].'；备注:'.$v['fk_h']);
        }
