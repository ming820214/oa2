...
class WeChat{
    ...
    public function getAccessToken(){
        $accessTokenFile = 'Data/access_token_cache.json'; // 缓存文件名
        if(!$this->getConfig('access_token')){
            ...
            if($update){
                // echo 'test';
                $resultJson = $this->_curl('https://qyapi.weixin.qq.com/cgi-bin/gettoken',
                    array('corpid' => $this->getConfig('corpid'),
     ...
     * @param  boolean  $fetchChild   是否递归获取子部门的成员
     * @param  integer $status        0获取全部成员，1获取已关注成员列表
     *                                2获取禁用成员列表，4获取未关注成员列表。status可叠加
     * @return array                  获取到的数据
     */
    public function getDepartmentMember($departmentId, $fetchChild = fasle, $status = 0){
        $fetchChild = (int)$fetchChild;
        $apiUrl = 'https://qyapi.weixin.qq.com/cgi-bin/user/simplelist';

        $result = $this->_curl($apiUrl, array(
     ...
     */
    public function jumpOAuth($uri){
        $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.
        $this->getConfig('corpid').'&redirect_uri='.
        urlencode($uri).'&response_type=code&scope=snsapi_base&state=WechatOAuth#wechat_redirect';
        header('Location: '.$url);
        exit;
    }
    ...
    public function getUserInfo($userid){
        ...
        $result = json_decode($this->_curl($apiUrl, array(
            'access_token' => $this->getAccessToken(),
            'userid'       => $userid,
        )), true);
     ...
     * @return array
     */
    public function deltUserInfo($userid){
