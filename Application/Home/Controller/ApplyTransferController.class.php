...
class ApplyTransferController extends HomeController {
    	...
    	if(IS_AJAX){
         		 ...
         		 if(get_school_name()!='集团'){
         		     ...
         		     if($stage != 1){
         		         if(session('auth_id') == '1283'){
         		             ...
         		             //          		         $w['area'] = '50';
         		             //          		         $w['state'] = 20;
         		             $w['_string']="((area='50' and state=20) or (state=10 and apply_school='13') or (state=30 and area=50))";
         		         }elseif(session('auth_id') == '89'){
         		             //王胜鑫
         		             if($stage != 1){
         		                 unset($w['apply_school']);
         		                 unset($w['area']);
         		                 $w['state'] = 30;
         		                 $w['area'] = array('in',[10,20.30,40]);
         		             }
    		...
    		if($flag){
    		    ...
    		    if($stage == 1){
    		        $map['_logic']='or';
    		        $map['_complex'] = $w;
    		        if(!(I('get.search')['state'] || I('get.search')['date1'] || I('get.search')['apply_school'] || I('get.search')['area'] ||  I('get.search')['terminal'] ||  I('get.search')['id'])){
    		            $map['add_user&state&is_del'] = array(session('auth_id'),array('elt',50),array('neq',1),'_multi'=>true);
    		        }
    		...
    		$data=M('applyTransfer')->where($map)->order('id desc')->field('id, state, apply_month,apply_school,area, name, sex, birth, political, education, profession, initial_entry, intent_transfer, cur_position, transfer_out, transfer_in, transfer_reason, reason_file, master_judge_flag, out_master_opinion, out_director_opinion, permit_flag, transfer_date, exchange_flag, transfer_result, why, create_time, update_time, is_del, add_user, add_user_name, terminal, descp')->limit(I('get.offset'),I('get.count'))->select();
    		  ...
    		  foreach ($data as &$vo){
    		        ...
    		        if(!$vo['master_judge_flag']){
    		            $vo['master_judge_flag'] = '';
    		        }
    		        
    		        if(!$vo['out_master_opinion']){
    		            $vo['out_master_opinion'] = '';
    		        }
    		        
    		        if(!$vo['out_director_opinion']){
    		            $vo['out_director_opinion'] = '';
    		        }
