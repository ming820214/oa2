...
class ComplaintController extends HomeController {
...
*/
	public function check(){
		if(IS_AJAX&&I('post.data')){
			if(D('ApplyComplaint')->check(I('post.type'),I('post.data')['id'],I('post.why')))$this->ajaxReturn('ok');
...
*/
    public function ajax_list(){
    	...
    	if(IS_AJAX){
    		    ...
    		    if(get_school_name()!='集团'){
    		        ...
    		        if(session('position_id') == '10'){
    		            ...
    		            if(session('auth_id') == '673'){
    		                //张鹏
    		                unset($w['apply_school']);
    		                $w['_string']="((area='10'and state>=20) or (apply_school='173'and state>=10))";
    		            }elseif(session('auth_id') == '439'){
    		                //何亮
    		                unset($w['apply_school']);
    		                $w['_string']="((area='40'and state>=20) or (apply_school='11'and state>=10))";
    		            }elseif(session('auth_id') == '651'){
    		                //王大鹏
    		                unset($w['apply_school']);
    		                
    		                $w['_string']="((area='30'and state>=20) or (apply_school='8'and state>=10))";
    		            }else{
    		...
    		$data=M('applyComplaint')->where($map)->order('id desc')->field('record',true)->limit(I('get.offset'),I('get.count'))->select();
    		...
    		$total=M('applyComplaint')->where($w)->count();
    ...
    public function ajax_repeatCheck(){
        if(IS_AJAX){
            ...
            $data=M('applyComplaint')->where($w)->field('record',true)->select();
            if(count($data)>0){
                $this->ajaxReturn(['state'=>'error','info'=>'该学员已经投诉过了！']);
            }else{
	...
	public function export(){
		...
		$dat=M('applyComplaint')->where($w)->order('id desc')->field('record',true)->select();
