...
Ycom = {
    page: {
        reset: function(obj, offset, count, total) {
            ...
            if(page > 1){
                var html = '<li><a href="javascript:void(0)" class="page" aria-label="Previous" data-page="'+ (page - 1) +'"><span aria-hidden="true">&laquo;</span></a></li>';
            }else{
                var html = '';
            }
            ...
            for(var i = page - 9; i <= lp; ++i){
                ...
                if(i == page){
                    ...
                    html += '<li><a class="page" data-page="'+ i +'" href="javascript:void(0)">'+ i +'</a></li>';
                }
            ...
            if(page < lp){
                html += '<li><a href="javascript:void(0)" class="page" aria-label="Next" data-page="'+ (page + 1) +'"><span aria-hidden="true">&raquo;</span></a></li>';
            }else{
                html += '';
            }
        ...
        bindEvent: function(obj, call) {
            $(obj).on('click', 'a.page', function() {
                call(wrapper.data('offset'), wrapper.data('count'), wrapper.data('total'));
            });
        },
    },
    ...
    table: function(obj, struct, data) {
        ...
        for (var row in data) {
            ...
            for (var col in struct) {
                html += '<td class="td_'+ col +'">'+  data[row][col] +'</td>';
            }
            html += '</tr>'
        }
        html += '</tbody>';
    ...
    fillForm: function(fo, data, ignore) {
        if(!ignore){
            ignore = '';
        }
    ...
    clearForm: function(fo, ignore) {
        ...
        $(form).find('input[type=hidden],\
                      input[type=text],\
                      input[type=number],\
                      textarea').not(ignore).val('');
        $(form).find('select').find('option:first').not(ignore).prop('selected','selected');
            // TODO radio checked。。。
    },
