...
class ChpController extends HomeController {
        ...
        if(I('post.id')){
            ...
            if($mod->save()){
            	
            	{
            		...
            		$wx= getWechatObj();
            		$wx->sendNewsMsg(
            				[$wx->buildNewsItem("请注意：您有一项CHP积分活动被更新！",$info,wx_oauth(C('WWW').'/oa_old/weixin.php/chp/chp'),'')],
            				['touser'=>$user],
            				C('WECHAT_APP')['TZTX']
            				);
	    	...
	    	if($mod->add()) {
	    		...
	    		
	    		{
	    			...
	    			$wx= getWechatObj();
	    			$wx->sendNewsMsg(
	    					[$wx->buildNewsItem("恭喜您，您又获得了CHP积分",$info,wx_oauth(C('WWW').'/oa_old/weixin.php/chp/chp'),'')],
	    					['touser'=>$user],
	    					C('WECHAT_APP')['TZTX']
	    					);
			...
			if($repeat){
				...
				if($mod->save()){
					
					{
						...
						if($repeat['flag']){
							$wx->sendNewsMsg(
									[$wx->buildNewsItem("请注意：您有一项CHP积分兑换被退回！",$info,wx_oauth(C('WWW').'/oa_old/weixin.php/chp/chp'),'')],
									['touser'=>$user],
									C('WECHAT_APP')['TZTX']
									);
						}else{
							$wx->sendNewsMsg(
									[$wx->buildNewsItem("请注意：您有一项CHP积分被删除！",$info,wx_oauth(C('WWW').'/oa_old/weixin.php/chp/chp'),'')],
									['touser'=>$user],
									C('WECHAT_APP')['TZTX']
									);
						}
			...
			if($repeat && $repeat['flag'] == 1){
				...
				if($mod->save()){
					
					{
						...
						$wx= getWechatObj();
						$wx->sendNewsMsg(
								[$wx->buildNewsItem("恭喜您：您有一份CHP积分兑换已通过！",$info,wx_oauth(C('WWW').'/oa_old/weixin.php/chp/chp'),'')],
								['touser'=>$user],
								C('WECHAT_APP')['TZTX']
								);
	...
	public function distory(){
		$mod = M('chpInfo');
		$condition['is_del'] = array('in','2,3'); //正常记录
	...
	public function distory_pager($id=0){
		$mod=M('chpInfo');
		...
		if(IS_AJAX && I('get.pageCount')){
			...
			$condition=I('post.');//获取json查询条件转换成php数组
			if(!$condition['is_del']){
				$condition['is_del'] = array('in','2,3'); //有效数据;
			}
