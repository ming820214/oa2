...
class WageController extends HomeController {
    //月初需要执行的操作
...
*/
    ...
    public function xc_xc1($school='',$class=1){
        if($_GET['save']){//数据存档
            $class=M('hw003.person_xc',null)->find(I('get.save'));
            ...
            $d['state']=1;
            M('hw003.person_xc',null)->where(array('id'=>I('get.save')))->save($d);die;
        }
        if(IS_AJAX){//数据保存
            $d[I('post.aa')]=I('post.info');
            M('hw003.person_xc',null)->where(array('id'=>I('post.id')))->save($d);die;
        }
        $map = self::map_search();
        $map['state'] = 1;
...
*/
    ...
    public function xc_5x1d(){
        if($_POST['record']){//五险一金缴纳记录
            $m=M('hw003.person_xc',null)->where(array('pid'=>I('post.record'),'type'=>0))->order('date desc')->field('date,5x1,5x2')->select();
            foreach ($m as $k => $v) {
                $m[$k]['5x3']=$v['5x1']+$v['5x2'];
            }
...
*/
    ...
    public function xc_fuli(){
        if($_POST['record']){//福利金存入记录
            $m=M('hw003.person_xc',null)->where(array('pid'=>I('post.record'),'type'=>0))->order('date desc')->field('date,fuli1,fuli2')->select();
            foreach ($m as $k => $v) {
        ...
        if(I('post.add')){//记录福利金提取
            $fl=M('hw003.person_xc_fulid',null)->find((int)$_POST['id']);
            $d['tiqu']=$fl['tiqu']+(float)$_POST['money'];
            $d['tiqu_date']=I('post.date');
            $d['record']='<提取时间：'.I('post.date').'提取金额：'.I('post.money').'备注：'.I('post.other').'>'.$fl['tiqu'];
