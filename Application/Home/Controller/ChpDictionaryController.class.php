...
class ChpDictionaryController extends HomeController {
    protected $config = array('app_type' => 'master','admin'=>'FinanceType');

    public function index() {
        $node = M("ChpDictionary");
        if (!empty($_POST['eq_pid'])) {
            $eq_pid = $_POST['eq_pid'];
        } elseif (!empty($_GET['eq_pid'])) {
            $eq_pid = $_GET['eq_pid'];
        } else {
            $eq_pid = $node -> where('pid=0 and is_del=1') -> order('sort asc') -> getField('id');
        }

        $this -> assign('eq_pid', $eq_pid);

        $list = $node -> where('pid=0 and is_del=1') -> order('sort asc') -> getField('id,name');
        $this -> assign('groupList', $list);

        $menu = array();
        $menu = $node -> where('is_del=1')  -> field('id,pid,name') -> order('sort asc') -> select();
        $tree = $this->list_to_tree($menu, $eq_pid);

        $model = M("ChpDictionary");
    ...
    public function read($id){
        $model = M('ChpDictionary');
        $vo = $model  -> where('is_del=1') -> find($id);
        /* if($vo['dept_id']){
    ...
    protected function _insert($name='ChpDictionary') {
        $model = D('ChpDictionary');
    ...
    protected function _update($name='ChpDictionary'){
        $id = $_POST['id'];
        $model = D("ChpDictionary");
        if (false === $model -> create()) {
            $this -> error($model -> getError());
    ...
    function winpop() {
        $menu = D("ChpDictionary") -> order('sort asc') -> select();
    ...
    function del($id){
        ...
        $list=M("ChpDictionary")->where($where)->select();

        if($list){
            $this->error('有子节点不能删除');
        }else{
        	$mo = M("ChpDictionary")->where("id=" . $id)->find();
        	if($mo){
        		$mo['is_del'] = 2;
        		if(M("ChpDictionary")->save($mo)){
        			$this->success('删除成功', 'ChpDictionary/index');
        		}
        	}
        }
       /*  $model = M("RoleNode");
